...
class Node:
	...
	def nodeCheck(self, dir):
		'''returns the next node given the interally kept current node and a direction'''
		...
		if self.currentNode == 0:
			...
			if dir == 'up':
				return 18
		elif self.currentNode == 200:		#special node for ghosts
			if dir == 'left':
				return 24
			if dir == 'right':
				return 25
		elif self.currentNode == 201:		#starting node for ghosts
			if dir == 'up':
				return 200
