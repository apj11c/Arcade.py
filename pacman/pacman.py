...
def pacmanGhostCollision(pacX, pacY, inkX, inkY, pinX, pinY, bliX, bliY, clyX, clyY):
	if (pacX == inkX and pacY == inkY) or (pacX == pinX and pacY == pinY) or (pacX == bliX and pacY == bliY) or (pacX == clyX and pacY == clyY):
		return True
	else:
		return False
...
while not crashed:
	...
	SCORE, GAMEOVER = foodMaker(myNode.getCurrentNode(), SCORE)
